...
export const CabinCard: React.FC<CabinCardProps> = ({ cabin, onBook }) => {
  ...
  const nextImage = (e: React.MouseEvent) => {
    e.stopPropagation();
    setCurrentImageIndex((prev) => (prev + 1) % cabin.pictures.length);
  };

  const prevImage = (e: React.MouseEvent) => {
    e.stopPropagation();
    setCurrentImageIndex((prev) => (prev - 1 + cabin.pictures.length) % cabin.pictures.length);
  };
  ...
  return (
    <div className="group flex flex-col overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-md transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
      {/* Galería de pictures / Media */}
      <div className="relative h-64 w-full overflow-hidden bg-gray-100">
        <Image
          src={cabin.pictures[currentImageIndex] || '/placeholder.jpg'}
          alt={cabin.name}
          fill
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        {/* comodities destacadas (primeras 3) */}
